<template>
    <div class="page">
        ...
        <div class="log">
            <ul>
                <li v-for="calc in calculations" v-bind:key="calc.id">{{calc.str}}</li>
            </ul>
        </div>
    </div>
</template>
            ...
            calculate() {
                if(this.prevNum !== null && this.isOperator) {
                    ...
                    if(this.operator.charAt(0) === '+') {
                        this.current = `${parseFloat(this.prevNum)+parseFloat(this.current)}`
                    ...
                    this.calculations.push({str: this.prevNum + ' ' + this.operator + ' ' + secNum + ' = ' + this.current, id: this.calculations.length});
